...
var today = new Date();
var beginDate = new Date(today.getTime() - 72 * 60 * 60 * 1000);;  // Default to Jan 1, 1970
var endDate = new Date(today.getTime() + 168 * 60 * 60 * 1000);  // Default to Jan 1, 2500
...
var titleRowKeys = ['title', 'description', 'location', 'starttime', 'endtime', 'guests', 'color', 'id', 'time',	'hour',	'minutes','totaltime'];
...
function convertCalEvent(calEvent) {
  ...
  convertedEvent.totaltime = (convertedEvent.endtime-convertedEvent.starttime)/(1000*3600);
  convertedEvent.hour =  Math.floor((convertedEvent.endtime-convertedEvent.starttime)/(1000*3600));
  convertedEvent.minutes = ((convertedEvent.endtime-convertedEvent.starttime)%(1000*3600)/(1000*60));
...
function syncFromCalendar() {
  ...
  var calendar = CalendarApp.getCalendarById(sheet.getSheetName());
