...
function validateTasks(textSearch, sheet) {
  ...
  let sentences = textMsg.split(". ");  // split into array of sentences
  let uniqueSentences = [...new Set(sentences)];  // remove duplicates using Set
  let message = uniqueSentences.join(". ");  // join array back into a string
...
function oneWeekTaskHourIsThirtySix(taskHours) {
  if (taskHours >= 36) {
    ...
    textMsg = textMsg + "The total task hours are smaller than to 36 for the week. \n";
  }
}
...
function weeklyScheduleTicketCountIsOne(sheet, row, column) {
  if (sheet.getRange(row, column + 4).getValue() === 1) {
    ...
    textMsg =
      textMsg + "The weekly schedule ticket count is not equal to 1 in the week. \n";
  }
}
...
function actionItemTicketCount(sheet, row, column) {
  if (sheet.getRange(row, column + 4).getValue() === 2) {
    ...
    textMsg = textMsg + "The Action Item ticket count is not equal to 2 in a week. \n";
  }
}
