...
function convertCalEvent(calEvent) {
  ...
  let convertedEvent = {
    ...
    creator_name: calEvent.getCreators(),
    rsvp_status: calEvent.getMyStatus(),
  };
  if (calEvent.getColor() === '') {
    convertedEvent.color = '7';
  }
...
function updateCalEvent(calEvent, convertedCalEvent, sheetEvent) {
  ...
  applyChanges(calEvent, updatedFields,sheetEvent);
...
function getUpdatedFields(convertedCalEvent, sheetEvent) {
  ...
  if (convertedCalEvent.color !== "" && convertedCalEvent.color !== String (sheetEvent.color)) {
...
function applyChanges(calEvent, updatedFields,sheetEvent) {
...
function syncFromCalendar() {
  ...
  if (sheet1[1][1] !== "") {
    ...
    beginDate = new Date(currentDate.setDate(currentDate.getDate() - currentDate.getDay() + 1));
    beginDate.setHours(0);
    beginDate.setMinutes(0);
    beginDate.setSeconds(0);
    endDate = new Date(beginDate.getFullYear(), beginDate.getMonth(), beginDate.getDate() + 7);
  }
  ...
  // for(let p=0;p<calEvents.length;p++){
  //   let myResponse = calEvents[p].getMyStatus();

  //   console.log("Event: " + calEvents[p].getTitle() + ", My RSVP Response: " + myResponse);
  // }
  ...
  setDateInColumnN()
  setAdjustedDateInColumnO()
  getWeekdayName()
  extractTitles()
  determinemeetingStatus()
...
function dataLenthIsOne(data, range, sheet, titleRow, titleRowKeys) {
  ...
  // let end_Date = convertDateFormat(sheet1[2][1])
}
...
function syncToCalendar() {
  ...
  let range = sheet.getDataRange();
  let data = range.getValues();
  let userMailId = sheet.getSheetName();
  if (data.length < 2) {
  ...
  setEventData(userMailId, data, sheet)
...
function eventDiffsIsPositive(
  ...
  if (eventDiffs > 0) {
    ...
    // process instead.
    ({ numUpdates, addEvent } = eventDiffsIsSmallerThanSeven(
      eventDiffs,
      numUpdates,
      calEvent,
      convertedCalEvent,
...
function eventDiffsIsSmallerThanSeven(
  eventDiffs,
  numUpdates,
  calEvent,
  convertedCalEvent,
  ...
  if (eventDiffs <=7 && eventDiffs !== EVENT_DIFFS_WITH_GUESTS) {
